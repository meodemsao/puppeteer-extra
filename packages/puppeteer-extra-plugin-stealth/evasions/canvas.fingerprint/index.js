...
class Plugin extends PuppeteerExtraPlugin {
  ...
  async onPageCreated(page) {
    await withUtils(this, page).evaluateOnNewDocument(
      this.mainFunction, this.opts
    )
  }

  onServiceWorkerContent(jsContent) {
    return withWorkerUtils(this, jsContent).evaluate(
      this.mainFunction, this.options
    )
  }
  ...
  mainFunction = (utils, opts) => {
    ...
    for (const {
      _Canvas, _CanvasRenderingContext2D, _Canvas_prototype_getContext, _Canvas_prototype_toDataURL
    } of classes) {
      // noinspection JSUnusedLocalSymbols
      ...
      const getNoisifyCanvas = canvas => {
        const {
          context: originalContext, contextIndex
        ...
        // }) + ' noise !');
        // Create a new canvas
        let canvasWithNoise = _Object.getPrototypeOf(canvas) ===
          ('undefined' !== typeof OffscreenCanvas && OffscreenCanvas.prototype)
        ...
        let newContext = _Object.getPrototypeOf(canvas) ===
          ('undefined' !== typeof OffscreenCanvas && OffscreenCanvas.prototype)
        ...
        newContext.putImageData(
          new ImageData(imageUint8Data, canvasWidth, canvasHeight),
          0,
          0
        )
      ...
      utils.replaceWithProxy(_Canvas.prototype, 'toDataURL', {
        apply(target, thisArg, args) {
          ...
          // noinspection JSUnusedLocalSymbols
          const { context, contextIndex } = utils.findRenderingContextIndex(
            canvas
          )
      ...
      utils.replaceWithProxy(
        ...
        'getImageData',
        {
          apply(target, thisArg, args) {
            let context = thisArg
            const contextIndex = utils.variables.renderingContextWithOperators.findIndex(
              e => e.context === context
            )
