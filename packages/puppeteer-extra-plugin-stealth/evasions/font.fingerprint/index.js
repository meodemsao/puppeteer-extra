...
const { buildFontSalt } = require('./fontSalt')

const fontSalt = buildFontSalt()
...
class Plugin extends PuppeteerExtraPlugin {
  ...
  mainFunction(utils, opts) {
    ...
    const allFonts = _Object.keys(fontSalt).map(e => e.toLowerCase())
    ...
    if (1) {
      ...
      for (const { _CanvasRenderingContext2D } of canvasAndContextClass) {
    ...
    if (1) {
      ...
      const hookStyle = domNode => {
        ...
        if (!hookConfig) {
          ...
          const styleDeclaration = _Reflect.apply(
            utils.cache.Descriptor.HTMLElement.prototype.style.get,
            domNode,
            []
          )
      ...
      const handleUserSetFontStyle = (cb, styleDeclaration, attr, value) => {
        ...
        if (hookConfig && attr && value) {
          const domNode = hookConfig.domNode
          ...
          if (targetAttr) {
            ...
            if (newFont) {
              ...
              const setPropertyCaller =
                utils.cache.Descriptor.CSSStyleDeclaration.prototype.setProperty
                  .value
    ...
    // font-face
    // noinspection JSUnresolvedVariable
    if (utils.cache.global.FontFace !== 'undefined') {
      ...
      utils.replaceWithProxy(FontFace.prototype, 'load', {
        async apply(thisArg) {
          ...
          try {
            // noinspection JSUnresolvedVariable
            result = await utils.cache.Descriptor.FontFace.prototype.load.value.call(
              thisArg
            )
          } catch (ex) {
            err = ex
    ...
    if (
      ...
      typeof globalThis.fonts !== 'undefined'
    ) {
