...
export function sleepRd(a: number, b: number) {
    const rd = _rd(a, b)
    return sleep(rd)
}
 ...
 */
export function _rd(min: number, max: number, pon = false): number {
    const c = max - min + 1
    return Math.floor(Math.random() * c + min) * (pon ? _pon() : 1)
}

export function _arrRd<T>(arr: T[]): T {
    if (!arr || !arr.length) {
...
export async function waitFor<T>(func: () => T, timeout: number): Promise<T | null> {
 ...
 */
export function twoBezier(t: number, p1: number[], cp: number[], p2: number[]): number[] {
    const [x1, y1] = p1
    const [cx, cy] = cp
    const [x2, y2] = p2
 ...
 */
export function threeBezier(t: number, p1: number[], cp1: number[], cp2: number[], p2: number[]): number[] {
    const [x1, y1] = p1
    const [x2, y2] = p2
    const [cx1, cy1] = cp1
    const [cx2, cy2] = cp2
...
export function makeFuncName(len = 4) {
    let result = ''
    for (let n = 0; n < len; ++n) {
